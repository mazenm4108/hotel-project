...
QMainWindow {
    ...
    roomNumberInput = new QSpinBox(tab2);
    roomNumberInput->setRange(100, 999);
    roomNumberInput->setValue(101);
    ...
    cancelRoomInput = new QSpinBox(tab4);
    cancelRoomInput->setRange(100, 999);
    cancelRoomInput->setValue(101);
    ...
    checkoutRoomInput = new QSpinBox(tab4);
    checkoutRoomInput->setRange(100, 999);
    checkoutRoomInput->setValue(101);
...
void MainWindow::bookRoom() {
    int rNum = roomNumberInput->value();
...
void MainWindow::cancelReservation() {
    int rNum = cancelRoomInput->value();
    for (int i = 0; i < Reservation_count; i++) {
...
void MainWindow::checkoutRoom() {
    int rNum = checkoutRoomInput->value();
    for (int i = 0; i < Reservation_count; i++) {
        if (reservations[i].room_number == rNum && reservations[i].isActive) {
            double price = 0;
            for (int j = 0; j < Max_rooms; j++) {
